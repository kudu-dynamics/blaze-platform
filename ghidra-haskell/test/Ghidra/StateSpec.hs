...
spec = describe "Ghidra.State" $ do
  context "loads a1 test binary" $ do
    ...
    (tempFilePath, dbFileExists) <- runIO $ do
      ...
      runGhidraOrError $ State.saveDatabase gs tempFilePath
  ...
  context "handles errors" $ do
    ...
    it "should be unable to load file that does not exist" $ do
      egs1 `shouldSatisfy` \case
        Right (Left (ImportByUsingBestGuessError msg)) ->
          "java.io.FileNotFoundException" `Text.isPrefixOf` msg
          || "java.io.IOException: File not found" `Text.isPrefixOf` msg
        _ -> False
