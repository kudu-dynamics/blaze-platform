...
## Version 0.26.0422
- Interprocedural dataflow analysis (`Flint.Analysis.Dataflow.*`): per-function param-to-output summaries composed bottom-up across the call graph, with first-class global-memory tracking (writes via `Store(CONST_PTR/FIELD_ADDR)`, reads via `LOAD`); positional param indices sourced from `Function.params`, multi-source arg mappings preserved across composition, `queryFlow` bridges across globals only when the current flow actually writes them
- `dataflow-summary` / `dataflow-scan` / `dataflow-query` shell commands and matching MCP tools. Query resolves externs (`free`, `strcpy`, …) as sinks via `findFunc`, not just internal functions
- Danger-sink scanner with severity-tagged sink list; libc-plumbing filter (`printf→vprintf`, `strdup→strcpy`, etc.) on by default — pass `--include-libc` / `include_libc=true` to see them
- Lazy + persistent dataflow caching on `CfgStore` (not `ShellState`) so flint CLI / flint-shell / flint-mcp share one copy: `dataflowCache` persists per-function intra summaries to LMDB in `.flintdb`, `composedDataflowCache` memoizes the bottom-up composition in memory; `getComposedDataflowStore` is the one-call entry point
...
## Version 0.26.0407
- C AST pattern matcher (`Flint.Analysis.CAst.Matcher`): `CExprPat`/`CStmtPat`/`ForPat` DSL with `LogicT` backtracking. Named bindings (`CBind`/`CBound`/`CUses`) enable cross-statement dataflow patterns; `runCheckOnStmts` walks every nested body via `allStmtLists` so patterns like `CSequence [CStar, target]` match regardless of control-flow depth. Findings track the actually-matched stmts (not the consumed prefix), deduped and capped at 20 per check with a truncation marker
- 11 built-in vulnerability patterns (`Flint.Analysis.CAst.Patterns`), severity + CWE tagged: 8 loop-focused (`loop-array-oob`, `loop-off-by-one`, `loop-accumulating-write`, `loop-use-after-free`, `loop-double-free`, `loop-memory-leak`, `loop-unbounded-input`, `loop-missing-bounds-check`) + 3 general (`dangerous-function`, `unchecked-alloc`, `format-string`)
- `check-cast` / `cast-scan` commands + MCP tools. `cast-scan` honors "analyzed functions only" by default (was silently scanning everything on empty cache); `--all` opts into the whole-binary scan. `cast-pattern-{add,list,remove,reset,save,load,show}` for runtime pattern definition via JSON; MCP `cast_pattern_add` bypasses the shell tokenizer so pretty-printed payloads with embedded whitespace survive
- Backend-agnostic C-AST types in new `Blaze.Types.CAst` (`CStmt`/`CExpr`/`CForInit`/`CCase`/`CAnn`/`AddrRange`); new `DecompImporter` typeclass split out of `BinaryImporter` (no more `Ghidra.Clang` in downstream code). Ghidra parser promotes literal-looking tokens to `CLitString`/`CLitInt` via `mkTokenValue` — previously string literals surfaced as `CIdent` with quotes included, causing `format-string` to fire on 100% of printf-family calls
- `flint-mcp` stdout safety: `Flint.Prelude.putText` writes to stderr so library debug output can't corrupt JSON-RPC frames; `Flint.Shell.Repl.shellOut` explicitly routes REPL output to stdout
