...
-- import Blaze.Pil.Solver as Solver
...
spec :: Spec
spec = describe "FIX THIS SPEC" $ do
  it "should get fixed later" $ do
    True `shouldBe` True

-- spec :: Spec
-- spec = describe "Blaze.Pil.SolverSpec" $ do
--   let solveSolver m = checkSatWith_ SBV.z3 m
...
--       bitVec = Ch.DSType $ Ch.TBitVector (bw 32)
...
--     --         constrain $ a `svEqual` (constInt 32 100 `pilAdd` constWord 32 50)

--     --       vars = [("a", CV (KBounded True 32) (CInteger 150))]
--     --   r <- runIO $ solveSolver cmd      
--     --   it "adds two numbers with different sign" $ do
--     --     r `shouldBe` Right (Sat (HashMap.fromList vars))

--     -- context "pilAdd" $ do
--     --   let cmd = do
--     --         a <- newSymVar "a" (KBounded True 32)
--     --         constrain $ constInt 32 150 `svEqual` (a `pilAdd` constWord 16 50)

--     --       vars = [("a", CV (KBounded True 32) (CInteger 100))]
--     --   r <- runIO $ solveSolver cmd
--     --   it "adds two numbers with different sign and width" $ do
--     --     r `shouldBe` Right (Sat (HashMap.fromList vars))
...
--       r <- runIO $ solveSolver cmd
--       it "lack of exception means matchBoundedWidth works." $ do
--         r `shouldBe` Right (Sat (HashMap.fromList vars))
...
--       r <- runIO $ solveSolver cmd
--       it "lack of exception means matchSign works." $ do
--         r `shouldBe` Right (Sat (HashMap.fromList vars))
...
--     --   let cmd = do
--     --         a <- newSymVar "a" (KBounded True 32)
--     --         constrain $ constInt 32 150 `svEqual` (a `pilSub` constWord 16 50)

--     --       vars = [("a", CV (KBounded True 32) (CInteger 200))]
--     --   r <- runIO $ solveSolver cmd
--     --   it "subtracts two numbers with different sign and width" $ do
--     --     r `shouldBe` Right (Sat (HashMap.fromList vars))
    
--     -- context "pilFPCeil" $ do
--     --   let cmd = do
--     --         a <- newSymVar "a" KDouble
--     --         b <- newSymVar "b" KDouble
--     --         constrain $ constFloat 4.33 `svEqual` a
--     --         constrain $ b `svEqual` pilCeil a

--     --       vars = [("a", CV KDouble (CDouble 4.33))
--     --               , ("b", CV KDouble (CDouble 5.0))]
--     --   r <- runIO $ solveSolver cmd
--     --   it "Rounds Fp number up to next Int" $ do
--     --     r `shouldBe` Right (Sat (HashMap.fromList vars))

--     -- context "pilFPFloor" $ do
--     --   let cmd = do
--     --         a <- newSymVar "a" KDouble
--     --         b <- newSymVar "b" KDouble
--     --         constrain $ constFloat 4.33 `svEqual` a
--     --         constrain $ b `svEqual` pilFloor a
--     --       vars = [("a", CV KDouble (CDouble 4.33))
--     --               , ("b", CV KDouble (CDouble 4.0))]
--     --   r <- runIO $ solveSolver cmd
--     --   it "Rounds Fp number up to next Int" $ do
--     --     r `shouldBe` Right (Sat (HashMap.fromList vars))
...
--     --         a <- newSymVar "a" KDouble
--     --         b <- newSymVar "b" KDouble
--     --         c <- newSymVar "c" KDouble
--     --         constrain $ constFloat 4.50 `svEqual` a
--     --         constrain $ constFloat 4.64 `svEqual` b
--     --         constrain $ c `svEqual` (a `pilFAdd` b)

--     --       vars = [("a", CV KDouble (CDouble 4.50))
--     --               , ("b", CV KDouble (CDouble 4.64))
--     --               , ("c", CV KDouble (CDouble 9.14))]
--     --   r <- runIO $ solveSolver cmd
...
--     --         c <- newSymVar "c" KDouble
--     --         constrain $ constFloat 4.50 `svEqual` a
--     --         constrain $ constFloat 1.50 `svEqual` b
--     --         constrain $ c `svEqual` (a `pilFDiv` b)

--     --       vars = [("a", CV KDouble (CDouble 4.50))
--     --               , ("b", CV KDouble (CDouble 1.50))
--     --               , ("c", CV KDouble (CDouble 3.0))]
--     --   r <- runIO $ solveSolver cmd
--     --   it "divides two floating point numbers" $ do
--     --     r `shouldBe` Right (Sat (HashMap.fromList vars))
    
--     -- context "pilRol" $ do
--     --   let cmd = do
--     --         a <- newSymVar "a" (KBounded False 32)
--     --         b <- newSymVar "b" (KBounded False 32)
--     --         c <- newSymVar "c" (KBounded False 32)
--     --         constrain $ constInt 32 10 `svEqual` a
--     --         constrain $ constInt 32 2 `svEqual` b
--     --         constrain $ c `svEqual` (a `pilRol` b)

--     --       vars = [("a", CV (KBounded False 32) (CInteger 10))
--     --               , ("b", CV (KBounded False 32) (CInteger 2))
--     --               , ("c", CV (KBounded False 32) (CInteger 40))]
--     --   r <- runIO $ solveSolver cmd
--     --   it "Rotates left value left by right value bits" $ do
--     --     r `shouldBe` Right (Sat (HashMap.fromList vars))
...
--     --         c <- newSymVar "c" (KBounded False 32)
--     --         constrain $ constInt 32 10 `svEqual` a
--     --         constrain $ constInt 32 32 `svEqual` b
--     --         constrain $ c `svEqual` (a `pilRol` b)

--     --       vars = [("a", CV (KBounded False 32) (CInteger 10))
--     --               , ("b", CV (KBounded False 32) (CInteger 32))
--     --               , ("c", CV (KBounded False 32) (CInteger 10))]
--     --   r <- runIO $ solveSolver cmd
--     --   it "Rotates left value left by right value bits; equivalent to mov" $ do
--     --     r `shouldBe` Right (Sat (HashMap.fromList vars))

--   context "declarePilVars" $ do
--     let runDecl tenvTuples = do
--           r <- runSolverWith SBV.z3 declarePilVars
--               ( emptyState, SolverCtx $ HashMap.fromList tenvTuples)
--           case r of
--             Left _ -> return $ Left ()
--             Right (_, ss) -> return . Right $ ( kindOf <$> ss ^. #varMap
--                                               , ss ^. #varNames
--                                               )
...
--       let tenv = [(pilVar "a", char)]
--           arg1 :: DSTExpression
--           arg1 = Ch.InfoExpression (Ch.SymInfo 8 $ Sym 3, Just char) 
--                  . Pil.VAR . Pil.VarOp $ pilVar "a"
--           -- arg1 :: DSTExpression
--           -- arg1 = Ch.InfoExpression (Ch.SymInfo 8 $ Sym 2, Just char)
--           --        . Pil.CONST . Pil.ConstOp $ 88
...
--           rvars = [("a", CV (KBounded False 8) (CInteger 12))]
--           errs = []
...
-- --                  . Pil.VAR . Pil.VarOp $ pilVar "a"
-- --           arg1 :: DSTExpression
-- --           arg1 = Ch.InfoExpression (Ch.SymInfo 64 $ Sym 2, Just float64)
...
--         let tenv = [ (pilVar "a", unsigned64) ]
--             ptr = constPtr 0xdeadbeef 4
--             -- stmts = [def "c" $ zx ( sx (load ptr 4) 4) 4]
