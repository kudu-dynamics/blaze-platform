...
matchStmtsIO tps pats = match' (solveStmtsWithZ3 Solver.AbortOnError) pats . mkPathPrep (Just tps)
...
spec = describe "Flint.Analysis.Path.Matcher" $ do
  context "matchNextStmt_" $ do
    let solver :: StmtSolver Identity
        ...
        matchNextStmt_' tryNextStmtOnFailure stmts pat = runIdentity . fmap (second isJust) . runMatcher solver (mkPathPrep Nothing stmts) $ do
  ...
  context "matchStmts" $ do
    let pureMatchStmts' tps pats stmts = pureMatch' pats $ mkPathPrep (Just tps) (stmts :: [Pil.Stmt])
        pureMatchStmts tps pats stmts = pureMatch pats $ mkPathPrep (Just tps) (stmts :: [Pil.Stmt])
