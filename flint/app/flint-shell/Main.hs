...
main = do
  ...
  withBackend (opts ^. #backend) fp $ \imp -> do
    ...
    base <- getBase imp
    shellState <- initShellState store base (not $ opts ^. #doNotUseSolver) (Just $ inspectAddress imp) (Just $ decompileFunctionText imp) (Just $ decompileFunctionAst imp) (Just $ \outPath -> saveToDb outPath imp) (Just $ getXrefsTo imp) (Just $ lookupGlobalSymbol imp)
